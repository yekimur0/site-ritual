...
const swiper_reviews = new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.swiper-review', {
  slidesPerView: 3,
  spaceBetween: 20,
  navigation: {
...
const swiper_news = new swiper__WEBPACK_IMPORTED_MODULE_2__["default"]('.swiper-news', {
  slidesPerView: 4,
  spaceBetween: 20,
  navigation: {
...
function setAttributeTabs(elements1, elements2) {
  elements1.forEach((el, i) => {
    el.setAttribute('data-tab-btn', `${i + 1}`);
  });
  elements2.forEach((el, i) => {
    el.setAttribute('data-tab-el', `${i + 1}`);
  });
}
setAttributeTabs(priceTabsBtns, priceTabsItems);
...
const listener = () => {
  document.body.addEventListener('click', e => {
    let target = e.target;
    if (target.classList.contains('tabs__btn')) changeTab(target);
    if (target.dataset.accordionItem) initAccordion(target);
    function changeTab(target) {
      let currentId = target.dataset.tabBtn;
      let element = document.querySelector(`[data-tab-el="${currentId}"]`);
